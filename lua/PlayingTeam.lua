...
function PlayingTeam:RespawnPlayer(player, origin, angles)
    ...
    if self:GetIsMarineTeam() then
        if origin == nil or angles == nil then
            ...
            if numSpawnPoints > 0 then
                ...
                marineSpawnPoint = GetRandomClearSpawnPoint(player, marineSpawnPoints)
    ...
    elseif self:GetIsAlienTeam() then
        if origin == nil or angles == nil then
        
            // Randomly choose unobstructed spawn points to respawn the player
            local alienSpawnPoint = nil
            local alienSpawnPoints = Server.team2SpawnList
            local numSpawnPoints = table.maxn(alienSpawnPoints)
            
            if numSpawnPoints > 0 then
            
                alienSpawnPoint = GetRandomClearSpawnPoint(player, alienSpawnPoints)
                if alienSpawnPoint ~= nil then
                
                    origin = alienSpawnPoint:GetOrigin()
                    angles = alienSpawnPoint:GetAngles()
